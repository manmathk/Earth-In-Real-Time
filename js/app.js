...
const els = {
  ...
  fastestMetric: document.getElementById("fastestMetric"),
...
function renderClock(elapsed) {
  ...
  els.sinceLabel.textContent = isToday ? "Counts so far on this UTC day" : "Full UTC day";
  ...
  els.netPeople.textContent = net ? net.display : "+0";
}
...
function renderSources() {
  els.sourcesList.replaceChildren(...METRICS.map((metric) => {
...
function updateEngagement(elapsed) {
  const cards = snapshotMetrics(elapsed, "all");
  const fastest = [...cards].sort((a, b) => b.perSecond - a.perSecond)[0];
  if (fastest) els.fastestMetric.textContent = `FASTEST: ${fastest.label.toUpperCase()}`;

  const seconds = Math.floor(elapsed);
  const cycle = seconds % 30;
  const challenges = [
    ["Stay for 10 seconds.", "Watch the numbers change without stopping."],
    ["Can you make it to 30?", "A few seconds is enough to change the planet’s counters."],
    ["Now pick a category.", "People. Travel. Planet. Digital. Which one surprises you?"],
    ["Don’t blink.", "The fastest counters are moving every second."],
    ["Beat your last watch.", "Leave this open and watch the gap grow."],
    ["Find the biggest number.", "Switch categories and see what dominates."],
  ];
  const item = challenges[Math.floor(seconds / 10) % challenges.length];
  els.challengeTitle.textContent = item[0];
  els.challengeText.textContent = item[1];
  const remaining = 10 - (seconds % 10);
  els.challengeCounter.textContent = remaining === 10 ? "10" : String(remaining);

  const headlines = [
    ["THE PLANET IS BUSY", "Births, flights, messages and emissions are all advancing."],
    ["SOMETHING JUST CHANGED", "Every second adds another slice of the world’s activity."],
    ["WATCH THE SCALE GROW", "A few more seconds means millions more in some counters."],
    ["LIVE HUMAN ACTIVITY", "The largest movement right now is happening in the digital layer."],
    ["EARTH DOESN’T PAUSE", "Even while this page sits still, the numbers keep climbing."],
  ];
  const [headline, detail] = headlines[Math.floor(seconds / 7) % headlines.length];
  els.eventHeadline.textContent = headline;
  els.eventDetail.textContent = detail;

  if (els.worldStage) {
    const intensity = Math.min(1, 0.45 + ((seconds % 10) / 10));
    els.worldStage.style.setProperty("--pulse-intensity", intensity.toFixed(2));
  }
}
...
function shiftDay(days) {
  ...
  els.pauseBtn.textContent = "PAUSE THE WORLD";
...
function setCategory(category) {
  ...
  document.querySelectorAll("[data-cat]").forEach((button) => button.setAttribute("aria-selected", String(button.dataset.cat === category)));
  render();
}
...
function togglePause() {
  ...
  els.pauseBtn.textContent = state.paused ? "RESUME THE WORLD" : "PAUSE THE WORLD";
...
function showToast(message) {
  ...
  window.setTimeout(() => { els.toast.hidden = true; }, 2200);
}

async function shareSnapshot() {
  const text = buildShareText(currentElapsed(), Date.now(), state.selectedDate);
  try {
    if (navigator.share) {
    ...
    if (error?.name !== "AbortError") showToast("Could not share snapshot");
  }
}
...
els.shareBtn.addEventListener("click", () => void shareSnapshot());
els.sourcesBtn.addEventListener("click", () => els.sourcesDialog.showModal());
els.sourcesDialog.querySelector(".dialog-close").addEventListener("click", () => els.sourcesDialog.close());
...
function tick() {
  if (state.paused) renderUtcClock();
  else {
    ...
    cards.forEach((card, index) => {
      ...
      if (node) node.querySelector(".stat-value").textContent = card.display;
    });
  }
...
function startParticles() {
  ...
  let width = 0; let height = 0; let particles = []; let last = performance.now();
  ...
  function paint(now) {
    ...
    for (const particle of particles) {
      ...
      ctx.shadowBlur = 10;
      ctx.globalAlpha = particle.alpha * 0.32;
      ctx.fillRect(particle.x - particle.r * .22, particle.y - particle.r * 7, particle.r * .44, particle.r * 7);
      ctx.beginPath(); ctx.globalAlpha = particle.alpha; ctx.arc(particle.x, particle.y, particle.r, 0, Math.PI * 2); ctx.fill();
    }
    ctx.globalAlpha = 1;
    requestAnimationFrame(paint);
  }
  resize(); window.addEventListener("resize", resize); requestAnimationFrame(paint);
}
